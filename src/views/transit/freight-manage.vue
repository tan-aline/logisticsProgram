      ...
      >
        ...
        >
          <span v-if="form.templateType === '经济区互寄'||form.templateType === '3'">
            <el-checkbox-group v-model="associatedCityList">
              <el-checkbox
                label="2"
                value="2"
              >京津冀</el-checkbox>
...
export default {
  ...
  methods: {
    ...
    async posttemplate(datas) {
      const newobj = this.deepClone(datas)
      // 打开对话框
      this.dialogVisible = true
      ...
      this.$refs.form.validate(async (isok) => {
        if (isok) {
          if (this.form.id) {
            ...
            // 发起添加请求
            if (this.associatedCityList.length > 1 && this.associatedCityList.find(item => item === '1')) {
              this.associatedCityList.splice(0, 1)
            }
            this.form.associatedCityList = this.associatedCityList
